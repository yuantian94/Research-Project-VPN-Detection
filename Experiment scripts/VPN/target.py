...
def ping_capture():
    ...
    os.system('sudo ping -c 3 '+VPNIP +' >> '+shortPingPath)
...
def trace_capture():
    ...
    os.system('sudo traceroute -w0.6 -m50 -T -n ' + VPNIP + ' >> '+shortTracePath)
    os.system(
        'echo \"------------------------------------------------------------------------------\" >> ' + shortTracePath)
...
def print_callback(pkt):
    try:
        ...
        if pkt.tcp.seq == '1' and  pkt.tcp.ack == '1' and pkt.ip.src == VPNIP and (pkt['tcp'].flags == '0x00000010' or pkt['tcp'].flags == '0x0010'):
            ...
            if packet_count == 5:
                ...
                time.sleep(5)
                sys.exit()
        if pkt.tcp.ack == '1' and pkt.tcp.seq == '0' and pkt.ip.dst == VPNIP and (pkt['tcp'].flags == '0x00000012' or pkt['tcp'].flags == '0x0012'):
...
if __name__ == "__main__":
    VPNIP = input("VPN Public IP address: ")
    clientIP = input("Client Public IP address: ")
    filename = input("Location configuration(Client-VPN-Target): ")
    my_ip = os.popen('curl -s ifconfig.me').readline()
    
    wireshark_filename = filename + '_' + clientIP + '-' + VPNIP + '-' + my_ip + '.pcap'
